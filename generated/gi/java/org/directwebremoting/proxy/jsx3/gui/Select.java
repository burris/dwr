...
public class Select extends org.directwebremoting.proxy.jsx3.gui.Block
{
     ...
     */
    public Select(ProxyHelper helper)
    {
        super(helper);
    }
    ...
    */
     ...
     * @return <code>TYPESELECT</code> or <code>TYPECOMBO</code>.
     *
    @SuppressWarnings("unchecked")
    ...
    */
     ...
     *
    @SuppressWarnings("unchecked")
    public String getDefaultText(Callback callback)
    {
        String key = // Generate some id
        ScriptSession session = WebContext.get().getScriptSession();
        Map<String, Callback> callbackMap = session.getAttribute(CALLBACK_KEY);
    ...
    */
    ...
    {
        ScriptBuffer script = new ScriptBuffer();
        script.appendData(getProxyHelper().getContext()).appendScript("setDefaultText(").appendData(strDefaultText).appendScript(");");
        getProxyHelper().getScriptProxy().addScript(script);
        return this;
    }
    ...
    {
        ScriptBuffer script = new ScriptBuffer();
        script.appendData(getProxyHelper().getContext()).appendScript("show(").appendScript(");");
        getProxyHelper().getScriptProxy().addScript(script);
    }
    ...
    {
        ScriptBuffer script = new ScriptBuffer();
        script.appendData(getProxyHelper().getContext()).appendScript("hide(").appendData(bFocus).appendScript(");");
        getProxyHelper().getScriptProxy().addScript(script);
    }
    ...
    */
     ...
     * @return the on-screen HTML element that was just given focus.
     *
    @SuppressWarnings("unchecked")
    ...
    */
     ...
     * @return the value or <code>null</code> if no record is selected.
     *
    @SuppressWarnings("unchecked")
    ...
    */
    ...
    /*
     * If this is a normal select box returns the value of the jsxtext attribute of the selected CDF record,
    ...
    */
    ...
    {
        ProxyHelper child = getProxyHelper().getChildHelper("redrawRecord(\"" + strRecordId + "\", \"" + intAction + "\").");
        try
        {
            Constructor<org.directwebremoting.proxy.jsx3.gui.Select> ctor = org.directwebremoting.proxy.jsx3.gui.Select.class.getConstructor(ProxyHelper.class);
            return ctor.newInstance(child);
        }
        catch (Exception ex)
        {
